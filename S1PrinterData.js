...
class S1PrinterData {
  ...
  constructor(trama) {
    console.log(
      "🚀 ~ file: S1PrinterData.js:210 ~ S1PrinterData ~ constructor ~ trama:",
      trama
    );
    // _arrayParameter = str(trama[1: -1]).split(chr(0X0A))
    trama = trama.substring(1, trama.length - 1);
    //split for any charater
    const _arrayParameter = trama.split(String.fromCharCode(0x0a));
    console.log(
      "🚀 ~ file: S1PrinterData.js:218 ~ S1PrinterData ~ constructor ~ String.fromCharCode(0x0a):",
      String.fromCharCode(0x0a)
    );

    console.log(
      "🚀 ~ file: S1PrinterData.js:215 ~ constructor ~ _arrayParameter:",
      _arrayParameter
    );
    if (_arrayParameter.length == 14) {
    }
  }
}
function chr(char) {
